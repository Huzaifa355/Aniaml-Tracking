...
void initModel() {
  ...
  // Verify output dimensions (should be [1, 2] for 2 classes)
  if (output->dims->size != 2 || output->dims->data[1] != 2) {
    Serial.println("ERROR: Output dimensions don't match expected [1, 2]!");
...
void fetchLastCounts() {
  ...
  const char* fields[2] = { "cowCount", "henCount" };
  int* counts[2]       = { &cowCount, &henCount };
  ...
  totalCount = cowCount + henCount;
  Serial.printf("Total restored counts → Cows: %d, Hens: %d, Total: %d\n",
                cowCount, henCount, totalCount);
}
...
void updateOLED() {
  ...
  display.setTextSize(1);
  display.setCursor(0, 0);
  display.printf("Cows:%d Hens:%d", cowCount, henCount);
...
void publishInfluxDB(int label, const String& direction) {
  ...
  static const char* names[2] = { "cow", "hen" };
...
void classifyAndUpdate(const String& direction) {
  ...
  const char* labels[] = {"Cow", "Hen"};

  // Apply softmax for proper probabilities
  softmax(outputData, 2);
  ...
  if (direction == "IN") {
    ...
    switch(bestIdx) {
      ...
      case 1: if (henCount > 0) { henCount--; decremented = true; } break;
    }
